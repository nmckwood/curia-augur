...
def _build_matrix(analysis):
    ...
    deltas_by_lad = {
        e["Local Authority District name"]: e["deprivation"]
        for e in analysis["constituencies"]
    }
...
def run(items, top_k=8):
    ...
    # k-means output is not used here, it is an additional step

    # get the file at the analysis key which is the output of 
    # data ingest pipeline -> machine learning pipeline
    # we essentially add the output payload to the array
    loaded = []

    # loop required as we run for all comparison years provided
    for item in items:
        # output of ML step
        analysis_key = item["analysis_key"]
        analysis = json.loads(io.read_output_text(analysis_key))
        loaded.append((analysis_key, analysis))

    prepared = []
    for analysis_key, analysis in loaded:
        # build a mtrix of LAD - deprivation indices
        cons, matrix, targets = _build_matrix(analysis)
        # split into 70/30 training and test
        train_idx, test_idx = _split(targets)
        # get the deprivation indices which indicate biggest changes
    ...
    # for each of our comparison years find the common X values
    # I.E the indices of deprivation
