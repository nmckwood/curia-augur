...
per-constituency block carries all 16, because the prediction stage rebuilds its
feature matrix from the analysis file alone and a decile-only block would leave it
training on eight constant-zero columns.
"""
        ...
        rows.append(
            {
                ...
                "deprivation": {
                    k: float(deprivation[k]) for k in ML_FEATURE_KEYS
                },
            }
        )
