...
class AnalysisSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final topFeatures =
        analysis.featureImportance.take(5).map((f) => _short(f.feature)).toList();

    final otherClusters =
        analysis.clusters.where((c) => !c.isHighChange).toList();
    final otherSize = otherClusters.fold<int>(0, (s, c) => s + c.size);
    final otherFlip = otherSize == 0
        ? 0.0
        : otherClusters.fold<double>(
                0, (s, c) => s + c.meanChangeFactor * c.size) /
            otherSize;

    final highFlipPct =
        high == null ? '—' : (high.meanChangeFactor * 100).toStringAsFixed(0);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Indices most characterizing the high-change cluster:',
